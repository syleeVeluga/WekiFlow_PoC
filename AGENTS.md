...
- Pipeline B, Graph Worker: 승인된 문서에서 Triple 방식 트리플을 추출해 MongoDB 그래프 컬렉션에 적재.
...
- **남은 작업(plans)** — `docs/plans/` (워크스페이스 인가 TODO, OKF 선택 PR-19/20).
- **이력(archive)** — 초기 Phase 0–4 아키텍처 설계(`docs/archive/01-architecture.md` 등), 프론트엔드 UI 구현 계획(`docs/archive/frontend-ui/`), 완료된 계획 18–21, OKF 조사·통합실행계획·구현기록(PR-01~18)은 `docs/archive/`에 보존돼 있다. 역사적 기록이며 일부 상호링크가 깨질 수 있다.
