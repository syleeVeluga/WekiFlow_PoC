...
- **남은 작업(plans)** — `docs/plans/`
  - OKF/enrichment 제품 흐름 재정렬: `docs/plans/okf-enrichment-product-flow/Overview.md`
  - 워크스페이스 인가 TODO: `docs/plans/workspace-authorization.md`
- **이력(archive)** — 초기 Phase 0–4 아키텍처 설계(`docs/archive/01-architecture.md` 등), 프론트엔드 UI 구현 계획(`docs/archive/frontend-ui/`), 완료된 계획 18–21, OKF 조사·통합실행계획·구현기록(PR-01~20)은 `docs/archive/`에 보존돼 있다. 역사적 기록이며 일부 상호링크가 깨질 수 있다.
...
OKF/WKF PR-01~20은 구현 완료되어 `docs/archive/okf-knowledge-standard/implementation/`에 보존돼 있다. 이후 OKF/enrichment 관련 작업은 먼저 `docs/plans/okf-enrichment-product-flow/Overview.md`의 갭 분석과 사용자-facing 단순화 원칙을 기준으로 삼는다.
