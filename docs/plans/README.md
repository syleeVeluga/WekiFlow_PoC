...
- [`dev-control-panel.md`](./dev-control-panel.md) — 에이전트 프롬프트·인자/한도·모델·정책(policy.yaml)을 재배포 없이 런타임 편집하는 슈퍼어드민 전용 패널. 접근은 역할 사다리와 분리된 직교 플래그 `isSuperAdmin`로 통제(WKF 후 권한 개편에 영향 없음). **진행 중: PR-21~23 완료, PR-24~25 남음.**
  - 후속 구현 전 점검: [`dev-control-panel/pr-24-25-impact-follow-up.md`](./dev-control-panel/pr-24-25-impact-follow-up.md)
