...
| PR | 제목 | 원본 §| 핵심 산출물 |
|----|------|------|-------------|
| [PR-21](./pr-21-superadmin-flag.md) | 직교 슈퍼어드민 플래그 | A | `isSuperAdmin` + `canAccessDevPanel`, `/api/admin/*` 게이트 |
| [PR-22](./pr-22-runtime-config-store.md) | 런타임 config 저장소 | B | `RuntimeConfigSchema`, repo, `loadRuntimeConfig` 머지, config API |
| [PR-23](./pr-23-prompt-injection-seam.md) | 프롬프트 주입 seam + 인자 배선 | C(+B) | 프롬프트 6키 seam, 워커 prompts·agentParams 주입 |
...
- **PR-21**: 선행 없음. 첫 작업.
- **PR-22**: PR-21 게이트 필요.
- **PR-23 / PR-24**: 둘 다 PR-22 필요. 서로 독립이라 **병렬 가능**.
