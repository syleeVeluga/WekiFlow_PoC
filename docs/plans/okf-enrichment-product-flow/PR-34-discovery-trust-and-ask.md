...
> Track T6 · 상태: 계획 · 선행: [PR-26](./PR-26-candidate-contract.md), [PR-27](./PR-27-candidate-model-and-trust-labels.md), [PR-33](./PR-33-knowledge-map.md)(backlinks 단서) · 근거: [`Overview.md`](./Overview.md) §3.4·§5.2-5, [`Gap-Analysis.md`](./Gap-Analysis.md) §3.7
> 외부 API 메모: 없음.
...
- 🔧 `apps/api/src/server.ts` — `/api/ask` 응답 스키마를 `{ answer }` → `{ answer, citations[], usedTrustLevels[] }`로 확장(SSE 이벤트 유지).
- 🔧 `packages/agent-tools/src/discovery.ts` — 검색 결과에 source/trust 메타 부착, 답변에 citation 연결, 우선순위 정렬.
...
- [ ] `/api/ask`가 출처와 신뢰 상태를 함께 반환한다.
- [ ] Ask 화면에서 답변·출처·신뢰 라벨이 보인다.
- [ ] `확인 필요` 출처 사용 시 답변에 표시된다.
- [ ] 답변 실패가 learner/conversation 흐름으로 환류된다.
