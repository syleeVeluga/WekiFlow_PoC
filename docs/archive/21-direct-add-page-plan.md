...
> 구현한다. 사용자가 주제를 배정하고 제목·워크스페이스·내용(직접 입력 또는 파일 업로드)·출처를 입력하면
...
내용 입력은 제목 필수 · 워크스페이스는 현재 워크스페이스 목록 · "P2 배치 검토" 안내는 **미반영**. 파일 업로드는
...
| 항목 | 결정 | 비고 |
| :--- | :--- | :--- |
| 레이아웃 | **공통 메타 + 탭 본문 교체** | 주제 배정·제목·워크스페이스·출처는 항상 표시, 입력 방식 탭이 `내용` 본문 영역만 교체 |
...
| 워크스페이스 목록 | `useUiStore().workspaces` 재사용 | 현재 기본값 `총무팀` |
...
**제출 활성 조건**: 주제 선택됨 && 제목 비어있지 않음 && 워크스페이스 선택됨 && (직접 입력 탭 → 내용 비어있지 않음 / 파일 탭 → 지원 포맷 파일 선택됨).
...
- **신규** [apps/web/src/components/add/AddPage.tsx] — 위 레이아웃. 재사용:
  ...
  - 워크스페이스 옵션: `useUiStore().workspaces`(현재 기본값 `총무팀`).
...
- [api/client.ts](../apps/web/src/api/client.ts):
  - `ingest()`(L91) 인자에 선택적 `topic?`, `workspace?`, `sourceLabel?` 추가(JSON 본문 포함).
  - **신규** `ingestFile(file, meta)` — `agentPreviewUpload`(L111) 패턴의 **전용 multipart fetch**
    ...
    `file` + `title`/`topic`/`workspace`/`sourceLabel` 전송.
...
- [apps/api/src/server.ts](../apps/api/src/server.ts):
  ...
  - `POST /api/ingest`(L199): 본문 `topic`/`workspace`/`sourceLabel` 통과 → `store.ingest`(`department`는 legacy 입력으로 호환).
  - **신규** `POST /api/ingest/file` — `request.isMultipart()` 처리(`readAgentPreviewInput` L78 패턴 준용),
    파일 추출 → `store.ingest({ title, contentMarkdown, topic, workspace, sourceLabel })`. 추출 공백이면 422, 미지원 포맷 415.
- [apps/api/src/store.ts](../apps/api/src/store.ts): `WekiFlowStore.ingest`(L60) 시그니처에 선택적
  `topic?`/`workspace?`/`sourceLabel?` 추가. `InMemoryWekiFlowStore.ingest`(L218)는 이를 `sourceRefs[].note`에 기록하고
...
### 2.5 공유 타입 (선택, 안전성)
- `packages/shared`: `IngestRequestSchema`(`title`, `contentMarkdown?`, `topic?`, `workspace?`, `department?: DepartmentSchema` legacy, `sourceLabel?`)를
...
- 워크스페이스 목록: `useUiStore().workspaces`(신규 백엔드 데이터 불필요).
...
- **단위(hermetic)**: [apps/api/src/server.test.ts](../apps/api/src/server.test.ts) 확장 —
  `POST /api/ingest`(JSON, topic/workspace 포함) → `{doc,job}`, doc.status `REVIEW`;
  `POST /api/ingest/file`(md/txt 멀티파트) → `REVIEW`; 미지원 포맷 415; 빈 추출 422.
- **수동 E2E**(dev 서버 또는 `run` 스킬):
  1. 로그인 → LNB `직접 추가` → 페이지 렌더(사이드바 시스템/사용자 주제, 워크스페이스 드롭다운, 입력 방식 탭).
  2. `+ 새 주제`로 사용자 주제 생성 → 사이드바 "직접 추가한 주제"·주제 배정 칩 즉시 반영, 삭제 동작.
  3. 직접 입력 탭: 주제+제목+워크스페이스+내용 → 검토 요청 → LNB `검토` 배지 +1, 트리에 `REVIEW` 문서, 입력 초기화.
...
- "P2 배치 검토" 안내 문구.
- 인입 문서의 실제 카테고리/워크스페이스 영속 모델 확장(메타는 sourceRefs note 수준 보존).
