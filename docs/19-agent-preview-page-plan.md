...
## 6. 미결 결정 (구현 중 확정 가능)
1. **재생 보존 정책** — 미리보기 `jobs` 행을 영구 보존할지, TTL을 둘지, "지우기" 액션을 둘지
   (`GET /api/agent-preview` 범위 제한).
2. **추출 상한** — pdf 최대 페이지/문자 수, 미리보기당 최대 청크 수로 OpenAI 비용/시간 제한
   (에이전트 루프는 이미 12 step 캡, 잘림은 UI에 표시).
