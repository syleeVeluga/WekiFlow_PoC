...
| 영역 | 목업 클래스 | 내용 |
| :--- | :--- | :--- |
| 로고 | `.sb-logo` | `WikiFlow`(em=purple-300) + 햄버거 SVG 마크 |
...
- [x] LNB가 목업과 시각적으로 일치(네이비·로고·워크스페이스·nav·시스템 섹션·사용자 푸터).
- [x] nav 클릭으로 `activePage` 전환 + `.ni.on` 활성 표시. 시스템 메뉴는 "준비 중" 스텁.
- [x] 검토 뱃지 = 검토수+멀티소스수, 조직 지식 뱃지 = 지식 총수 (data 훅 구독, 라이브).
- [x] 문서 트리가 카테고리별로 색점·문서수와 함께 렌더, `modCount>0` 문서에 주황 업데이트 점.
- [x] 카테고리 펼침/접힘(`treeOpen`), 문서/카테고리 클릭이 `openDoc`/`openCategory` 디스패치.
- [x] 검색 입력이 문서명 필터 + 매칭 카테고리 자동 확장(파생 필터).
