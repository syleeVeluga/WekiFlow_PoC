...
- [ ] 그리드/카드가 목업과 일치(상태 점·AI태그·카테고리 칩·작성자·참조수).
- [ ] 4필터(담당자·주제·AI태그·상태) + 검색 + 정렬 3종 + 뷰 토글이 **합성** 동작(교차 필터).
- [ ] AI 태그 배너 + 모달 승인/반려가 문서 `aiTags`·제안 카운트를 갱신.
- [ ] 카테고리 관리 추가/삭제 동작 + 사용자 주제 삭제 시 `미분류` 재배정.
- [ ] 카테고리별 뷰·통합 보기가 카테고리 전체 문서를 스크롤 렌더, 진입/복귀 동작.
- [ ] 단일 문서가 트리·카드·다이제스트에서 진입, 탭 전환, 편집→저장 시 **트리 업데이트 점·카드 수정수 갱신**, 챗봇 토글 토스트.
- [ ] 트리 클릭-스루(카테고리→통합보기, 문서→단일문서)가 Phase 3와 연결되어 완전 동작.
